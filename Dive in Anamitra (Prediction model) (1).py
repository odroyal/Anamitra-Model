...
# The value which is close to 1 or 1 are more like to have good relation for customer to purchas the product, and if the value is -1 or shows Nan then it has no relation with the particular feature. Above, QTY is empty because it has no relation with any other feature
...
# Optional: Visualizing forecast components (trend, weekly, yearly seasonality)
fig2 = model.plot_components(forecast)
plt.show()

# Printing the forecasted values
forecast[['ds', 'yhat', 'yhat_lower', 'yhat_upper']].tail(forecast_horizon)
...
# Label encode categorical feature if needed
label_encoder = LabelEncoder()
df['Gross Weight'] = label_encoder.fit_transform(df['Gross Weight'])
df['Category'] = label_encoder.fit_transform(df['Category'])

# Splitting the data into features (X) and target (y)
X = df[['Month', 'Year', 'Gross Weight','Category']]  # Assuming 'Month', 'Year', and 'Gross Weight' are features
y = df['Sales Amount']  # Assuming 'Sales Amount' is the target variable
...
model = RandomForestRegressor(n_estimators=100, random_state=42)  # Using Random Forest model
...
for actual, predicted in zip(y_train, y_pred_train):
